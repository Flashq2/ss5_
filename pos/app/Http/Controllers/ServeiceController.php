...
class service
{
        ...
        try {
            ...
            if ($model_name) {
                if (hasTableHelper($model_name) && hasColumnHelper($model_name, $code) && hasColumnHelper($model_name, $desc)) {
                    ...
                    if ($model_name == 'chart_of_account')
                        $_list = $_list->where('account_type', 'Posting');
    ...
    public function generateNo($no_series_code, $ref)
    {
        ...
        if (!$no_line)
            return "error_no_series";
        if ($no_line->increment == 0 || $no_line->increment == "")
            $no_line->increment = 1;
    ...
    public function generateNoV2($no_series_code, $ref)
    {
        ...
        if ($no_series_code) {
            ...
            if (!$no_series) {
                ...
                if ($no_series->allow_manual == 'Yes')
                    return ['status' => 'success', 'no_serial' => $no_series_code];
                else {
    ...
    public static function CollectionCulumn($salesperson)
    {
        ...
        if ($customer)
            $sales_kpi_analysis_code = $customer->sales_kpi_analysis_code;
    ...
    {
        if ($is_print == 'excel') {
            if ((float) $number == 0 || (float) $number == -0 || $number == '')
                return '0';
        } else {
            if ((float) $number == 0 || (float) $number == -0 || $number == '')
                return '-';
    ...
    public static function number_formattor_database($number, $format)
    {
        ...
        switch (strtolower($format)) {
            ...
            case 'vat_amount':
                ...
                break;
            case 'date':
                if ($number == null || $number == '' || $number == '1900-01-01')
                    $result = '1900-01-01';
                else if ($number == '2500-01-01')
                    $result = '2500-01-01';
                else
                    $result = Carbon::parse($number)->toDateString();
                break;
            case 'datetime':
                if ($number == null || $number == '' || $number == '1900-01-01 00:00:00')
                    $result = '1900-01-01 00:00:00';
                else if ($number == '2500-01-01 00:00:00')
                    $result = '2500-01-01 00:00:00';
                else
                    $result = Carbon::parse($number)->toDateTimeString();
                break;
            default:
    ...
    public static function currency_formattor_database($currency, $number, $format)
    {
        ...
        if ($currency) {
            ...
            if (service::toDouble($currency->minimum_note) > 0) {
                ...
                if ($variant_amount > 0)
                    $number = service::toDouble($number) - service::toDouble($variant_amount);
            }
        } else {
            switch ($format) {
                case 'price':
    ...
    {
        if (!$date || $date == '1900-01-01' || $date == '2500-01-01')
        ...
        if ($field['field_data_type'] == 'decimal') {
            ...
            try {
                ...
                if (strtoupper($value) == 'T' || $value == 'TODAY') {
                    ...
                    if ($calc_date_formula != '1900-01-01')
                        return $calc_date_formula;
                ...
                if (strtoupper($value) == 'T' || strtoupper($value) == 'TODAY') {
                    ...
                    if (strpos($value, '/') !== false) {
                        ...
                        if (strlen($dateparts[0]) == 4) {
                            ...
                            if ($month) {
                                if (strlen($month) > 2)
                                    $month = $this->convertMonthname2Number($month);
                            }
            ...
            if (strtoupper($value) == 'T' || strtoupper($value) == 'TODAY') {
                ...
                if (strpos($value, '/') !== false) {
                    ...
                    if (strlen($dateparts[0]) == 4) {
                        ...
                        if ($month) {
                            if (strlen($month) > 2)
                                $month = service::_convertMonthname2Number($month);
                        }
                        return date_format(Carbon::createFromDate($year, $month, isset($dateparts[2]) ? $dateparts[2] : null), 'Y-m-d');
                    } else {
                        $year = isset($dateparts[2]) ? $dateparts[2] : null;
                ...
                if (strtolower($field->input_type) == 'flowfield') {
                    if (strtolower($field->flow_field_method) == 'formula') {
                        ...
                        if (strtolower($field->flow_field_method) == 'sum') {
                            $reverse_sign = '';
                            if (strtolower($field->flow_field_reverse_sign) == 'yes')
                                $reverse_sign = ' * (-1)';
    ...
    {
        if ($ex->getMessage() == '' || $ex->getMessage() == 'Unauthenticated.')
            return;
        ...
        if (config('app.env') == 'production' && $is_send_telegram)
            $this->sendMessageTelegramBot($ex, $url, $msg);
    ...
    public function DoImportExcel($values, $table_name, $data, $modal_name, $ref = '', $header = null)
    {
        ...
        if (Auth::user()->user_type != 'Full User')
            return 'false';
                ...
                if ($record) {
                    ...
                    foreach ($table_fields as $table_field) {
                        if ($table_field->field_name != $field_pri_key) {
                            ...
                            if (!array_key_exists($table_field->field_name, $values))
                                continue;
    ...
    public function NewRecordImport($table_fields, $field_pri_key, $values, $modal_name, $ref = '', $header = null, $excel_fiels, $primary_key_value = null, $excel_line_no)
    {
        try {
            ...
            if (strtoupper($primary_key_value) == '_AUTO') {
                if ($ref) {
                    $serviceInstance = new \App\Services\service(); // Create an instance
                    $new_pri_value = $serviceInstance->generateNo('', $ref); // Call the method on the instance
                }
            } else {
            ...
            if ($modal_name == 'App\Models\Administration\ApplicationSetup\Item') {
                ...
                if (isset($values['ship_to_code']))
                    $is_existed = $is_existed->where('ship_to_code', $values['ship_to_code']);
                $is_existed = $is_existed->first();
                if ($is_existed) {
                    $description = "Customer: " . $values['no'] . ' and Salesperson:' . $values['salesperson_code'];
                    if (isset($values['ship_to_code']))
                        $description .= "Ship to Code: " . $values['ship_to_code'];
                ...
                $ref = "Payment Journal";
                if ($values['journal_type'] == "Purchase Journal")
                    $ref = "Purchase Journal";
            ...
            foreach ($table_fields as $table_field) {
                ...
                //     }
                // }
                if (!isset($values[$table_field->field_name]))
                    continue;
            ...
            if ($modal_name == 'App\Models\Sales\Transaction\SaleLine') {
                ...
                if ($record->type == "Item" && $record->isItemTracking() == true) {
                    ...
                    if ($item->item_tracking_code == 'LOTALL')
                        $item_ledger_entries = $item_ledger_entries->orderBy("expiration_date", "ASC")->get();
                    else
            ...
            if ($modal_name == 'App\Models\Warehouse\Transaction\ItemJournalLine' && $record->journal_type == "Phy. Inventory Journal") {
                if ($record->isItemTracking() == true) {
                    ...
                    if (isset($values['lot_no']))
                        $record->lot_no = $values['lot_no'];
                    $org_record = $record;
        ...
        try {
            ...
            if ($item_specifications) {
                foreach ($item_specifications as $item_specification) {
                    ...
                    $document_type = 'your_document_type_value'; // Replace with the actual value
                    $vendor_no = purchase_header->vendor_no; // Make sure $purchase_header is assigned a value
                    $line_no = $line_no + 10; // Make sure $line_no is assigned a value
                    $refer_line_no = 'your_refer_line_no_value'; // Replace with the actual value

                    $specification->document_type = $document_type;
                    $specification->vendor_no = $vendor_no;
        ...
        if ($user_setup->salesperson_code) {
            ...
            if (count($customer_filters) > 0) {
                ...
                foreach ($customer_filters as $customer_filter) {
                    ...
                    if ($customer_filter->field_condition == 'or')
                        $is_or_existed = true;
                    $i++;
                }
                if ($is_or_existed) {
        ...
        try {
            ...
            if (!$header) {
                if ($is_return_arr_key)
                    return ['sub_total' => 0, 'total_disc' => 0, 'total_vat' => 0, 'amount_due' => 0, 'total_amount_text' => 0, 'total_vat_plt' => 0];
                else
            ...
            if ($header->price_include_vat == 'Yes') {
                foreach ($lines as $line) {
                    ...
                    if ($line->type == 'Item') {
                        if ($line->price_option == 'By Length') {
                            if (service::toDouble($line->length) > 0)
                                $price_option_ration = service::toDouble($line->length);
                        } else if ($line->price_option == 'By Width') {
                            if (service::toDouble($line->width) > 0)
                                $price_option_ration = service::toDouble($line->width);
                        } else if ($line->price_option == 'By Height') {
                            if (service::toDouble($line->height) > 0)
                                $price_option_ration = service::toDouble($line->height);
                        } else if ($line->price_option == 'By Cubage') {
                            if (service::toDouble($line->cubage) > 0)
                                $price_option_ration = service::toDouble($line->cubage);
                        } else if ($line->price_option == 'By Weight') {
                            if (service::toDouble($line->weight) > 0)
                                $price_option_ration = service::toDouble($line->weight);
                        } else if ($line->price_option == 'By Qty Per Unit') {
                            if (service::toDouble($line->qty_per_unit_of_measure) > 0)
                                $price_option_ration = service::toDouble($line->qty_per_unit_of_measure);
                        ...
                        if ($item)
                            $sales_amount_calc_option = $item->sales_amount_calc_option;
                    }
                    if ($line->type != 'Text') {
                ...
                foreach ($lines as $line) {
                    ...
                    if ($line->type != 'Text') {
                        if ($sales_amount_calc_option == 'Quantity 2') {
                            ...
                            if ($has_col && $app_setup->price_decimal_after_dis != 'Yes')
                                $has_col = false;
                            if ($has_col)
                                $customer = Customer::select('price_decimal_after_dis')->where('no', $header->customer_no)->first();
                ...
                if (getSettingHelper(14, 'Yes') && $disc_percentage_diff_value == 1 && count($lines) > 0) {
                    ...
                    if ($get_first)
                        $discountTotal = (service::toDouble($sub_total) * $get_first->discount_percentage) / 100;
                }
                if ($is_return_arr_key) {
    ...
    public static function totalSalesAmount($header, $lines)
    {
        if (!$header) {
            return array(
                'amount_excluded_vat' => 0,
                'discount_amount' => 0,
                'vat_amount' => 0,
                'amount_included_vat' => 0,
                'amount_excluded_vat_after_discount' => 0
            );
        }
        if ($header->price_include_vat == 'Yes') {
            $subTotal = count($lines) > 0 ? $lines->sum(function ($r) {
            ...
            return array(
                'amount_excluded_vat' => $subtotalExcludeVAT,
                'discount_amount' => $discountTotal,
                'vat_amount' => $vatTotal,
                'amount_included_vat' => $amountDue,
                'amount_excluded_vat_included_discount' => $subtotalExcludeVAT + $discountTotal
            );
        } else {
            $subTotal = count($lines) > 0 ? $lines->sum(function ($r) {
                return service::toDouble($r->quantity) * service::toDouble($r->unit_price);
            ...
            return array(
                'amount_excluded_vat' => $subTotal,
                'discount_amount' => $discountTotal,
                'vat_amount' => $vatTotal,
                'amount_included_vat' => $amountDue,
                'amount_excluded_vat_included_discount' => $subTotal + $discountTotal
            );
        }
    }
    ...
    public static function totalPOSSalesAmount($header, $lines, $other_currency_rate)
    {
        ...
        if ($header->price_include_vat == 'Yes') {
            ...
            return array(
                'amount_excluded_vat' => $amount_excluded_vat,
                'discount_amount' => $discount_amount,
                'vat_amount' => $vat_amount,
                'amount_included_vat' => $amount_included_vat,
                'sub_total_excluded_vat' => $sub_total_excluded_vat,
                'inv_discount_percentage' => $inv_discount_percentage,
                'inv_discount_amount' => $inv_discount_amount,
                'payment_amount' => $payment_amount,
                'amount_included_vat_other' => $amount_included_vat_other,
                'change_in_usd' => $change_in_usd,
                'change_in_other' => $change_in_other,
                'amount_excluded_vat_included_discount' => $amount_excluded_vat + $discount_amount + $inv_discount_amount
            );
        } else {
            $sub_total_excluded_vat = count($lines) > 0 ? $lines->sum(function ($r) {
                return service::toDouble($r->quantity) * service::toDouble($r->unit_price);
    ...
    public function isExistGLEntry($record, $document_no, $account_no, $type = '')
    {
        $user = Auth::user();
        $app_setup = $user->app_setup;
        if ($app_setup->sales_gl_compress != 'Yes' || $type == '')
            return null;
        $criteria = '';
    ...
    public function checkPasswordDownload($password, $remember = 'No')
    {
        if ($remember == 'Yes') {
            if (!Session::get('remember_password'))
                Session::put('remember_password', $password);
    ...
    public static function calcDateFormula($value)
    {
        try {
            ...
            if (substr($string, 0, 1) === "T") { //Today
                if (!in_array(substr($string, 1, 1), $arr_symbol))
                    return "1900-01-01";
                ...
                if (!in_array(substr($string, 0, 2), $arr_special_str) || !in_array(substr($string, 2, 1), $arr_symbol))
                    return "1900-01-01";
            ...
            if (count($arr_value) <= 1 || $arr_value == 'false')
                return "1900-01-01";

            if (strtoupper($strStart) == 'T' || strtoupper($strStart) == 'TODAY')
                $date_value = Carbon::today()->toDateString();
            elseif (strtoupper($strStart) == 'TO' || strtoupper($strStart) == 'TOMORROW')
                $date_value = Carbon::tomorrow()->toDateString();
            elseif (strtoupper($strStart) == 'Y' || strtoupper($strStart) == 'YESTERDAY')
                $date_value = Carbon::yesterday()->toDateString();
            elseif (strtoupper($strStart) == 'CM')
                $date_value = Carbon::now()->endOfMonth()->toDateString();
            elseif (strtoupper($strStart) == 'CW')
                $date_value = Carbon::now()->endOfWeek();
            elseif (strtoupper($strStart) == 'SW')
                $date_value = Carbon::now()->startOfWeek();
            elseif (strtoupper($strStart) == 'CY')
                $date_value = Carbon::now()->endOfYear();
            ...
            foreach ($arr_value as $key => $value) {
                if (!is_numeric($value) && in_array($value, $arr_symbol)) {
                    ...
                    if (substr($next_value, -1) === "D") {
                        if ($value === "+")
                            $date_value = Carbon::parse($date_value)->addDays($get_date_number);
                        if ($value === "-")
                            $date_value = Carbon::parse($date_value)->subDays($get_date_number);
                    } else if (substr($next_value, -1) === "W") {
                        if ($value === "+")
                            $date_value = Carbon::parse($date_value)->addWeeks($get_date_number);
                        if ($value === "-")
                            $date_value = Carbon::parse($date_value)->subWeeks($get_date_number);
                    } else if (substr($next_value, -1) === "M") {
                        if ($value === "+")
                            $date_value = Carbon::parse($date_value)->addMonths($get_date_number);
                        if ($value === "-")
                            $date_value = Carbon::parse($date_value)->subMonths($get_date_number);
                    } else if (substr($next_value, -1) === "Y") {
                        if ($value === "+")
                            $date_value = Carbon::parse($date_value)->addYear($get_date_number);
                        if ($value === "-")
                            $date_value = Carbon::parse($date_value)->subYear($get_date_number);
                    } else {
                        return "1900-01-01";
                    }
                }
    ...
    public function getModels($path)
    {
        ...
        foreach ($results as $result) {
            if (in_array($result, ['.', '..', 'VposSaleData.php']))
                continue;
            ...
            if (is_dir($filename)) {
                ...
                if (strpos($filename, '.php')) {
                    ...
                    if ($config['name'] == 'company')
                        $out[$model->getTable()] = $model_path;
                }
            }
        }
    ...
    public function checkCustomerCredit($customer, $payment_amount, $document_no, $date = "")
    {
        ...
        if ($customer->credit_limited_type == "Balance" && $total_agin_amount != '') {
            ...
            $customer_ledger_enties = DB::connection('company')->table('v_customer_ledger_entry')
                ...
                ->count();
            if ($customer_ledger_enties >= $total_agin_amount)
                return 2;
        } elseif ($customer->credit_limited_type == 'No Credit') {
            if ($payment_amount == '' || service::toDouble($payment_amount) == 0)
                return 3;
        }
        ...
        if (service::toDouble($payment_amount) > service::toDouble($total_amount))
            return 4;
    }
    ...
    {
        if (abs(service::toDouble($amount)) != abs(service::toDouble($line_remaining_amount)))
            return 0;
        $factor = 1;
        ...
        if (abs(service::toDouble($amount_to_apply_header_lcy)) != abs(service::toDouble($line_remaining_amount_lcy))) {
            ...
            if (abs(service::toDouble(service::number_formattor($amount_to_apply_header_lcy, 'amount'))) != abs(service::toDouble(service::number_formattor($line_remaining_amount_lcy, 'amount')))) {
                ...
                if (service::toDouble($gain_loss_amount_line) < 0)
                    $factor = $factor * (-1);
    ...
    public static function checkitemNew($item_no)
    {
        ...
        if (!$purcahse_data)
            $is_new_item = 'Yes';
        if ($purcahse_data && Carbon::parse($purcahse_data->posting_date)->diffInDays(Carbon::now()) < 1)
            $is_new_item = 'Yes';
        return $is_new_item;
    }
